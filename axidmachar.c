...
static unsigned bufsize_rd = 1 * 1024 * 1024;
static unsigned bufsize_wr = 1 * 64 * 1024;
static unsigned bufcount_rd = 200;
static unsigned bufcount_wr = 1;
module_param(bufsize_rd, int, S_IRUGO);
module_param(bufsize_wr, int, S_IRUGO);
module_param(bufcount_rd, int, S_IRUGO);
module_param(bufcount_wr, int, S_IRUGO);
...
/* static ssize_t charwr_read(struct file *, const char *, size_t, loff_t *); */
...
static int xilinx_dma_alloc_chan_resources(struct dma_chan *dchan) {
	...
	if (!chan->desc_pool && (chan->xdev->dma_config->dmatype != XDMA_TYPE_AXIDMA)) {
...
//		dev_info(dev, "Not resuming. [p%d.%d-w%d.%d]%d(%d)\n",
//		        rx_buf_process_index, dma_rx_bufs[rx_buf_process_index].bs,
//		        rx_buf_store_index, dma_rx_bufs[rx_buf_store_index].bs,
//		        trans_dma_cnt, trans_dma_cnt - trans_read_cnt);
...
//static irqreturn_t modified_tx_irq_handler(int irq, void *data) {
//	struct xilinx_dma_chan *chan = data;
//	dev_info(dev, "IRQ: modified_tx_irq_handler\n");
//	return IRQ_HANDLED;
//}
...
//	dev_info(dev, "IRQ state:[%d] CR=0x%08x SR=0x%08x\n DA=0x%08x,LEN=0x%08x\n",
//	        rx_buf_store_index, dma_ctrl_read(chan, XILINX_DMA_REG_DMACR), status, dma_ctrl_read(chan, XILINX_DMA_REG_SRCDSTADDR), length);
...
static void append_desc_queue(struct xilinx_dma_chan *chan, struct xilinx_dma_tx_descriptor *desc) {
	...
	if (chan->xdev->dma_config->dmatype == XDMA_TYPE_VDMA) {
		...
		cdma_tail_segment = list_last_entry(&tail_desc->segments, struct xilinx_cdma_tx_segment, node);
...
xilinx_vdma_dma_prep_interleaved(struct dma_chan *dchan, struct dma_interleaved_template *xt, unsigned long flags) {
	...
	hw->stride = (xt->sgl[0].icg + xt->sgl[0].size) << XILINX_DMA_FRMDLY_STRIDE_STRIDE_SHIFT;
	hw->stride |= chan->config.frm_dly << XILINX_DMA_FRMDLY_STRIDE_FRMDLY_SHIFT;
 ...
 * Return: '0' on success and failure value on error
 */
int xilinx_vdma_channel_set_config(struct dma_chan *dchan, struct xilinx_vdma_config *cfg) {
	struct xilinx_dma_chan *chan = to_xilinx_chan(dchan);
	u32 dmacr;
...
static int xilinx_dma_chan_probe(struct xilinx_dma_device *xdev, struct device_node *node, int chan_id) {
	...
	if (of_device_is_compatible(node, "xlnx,axi-vdma-mm2s-channel") || of_device_is_compatible(node, "xlnx,axi-dma-mm2s-channel")
		...
		if (xdev->dma_config->dmatype == XDMA_TYPE_VDMA) {
			...
			if (xdev->flush_on_fsync == XILINX_DMA_FLUSH_BOTH || xdev->flush_on_fsync == XILINX_DMA_FLUSH_S2MM)
				chan->flush_on_fsync = true;
		}
	} else {
...
//		dev_info(xdev->dev, "registering RX interrupt modified_rx_irq_handler\n");
	err = request_irq(chan->irq, modified_rx_irq_handler, IRQF_SHARED, "xilinx-dma-controller", chan);
//	} else if (of_device_is_compatible(node, "xlnx,axi-dma-mm2s-channel")) {
//		dev_info(xdev->dev, "registering TX interrupt modified_tx_irq_handler\n");
//		err = request_irq(chan->irq, modified_tx_irq_handler, IRQF_SHARED, "xilinx-dma-controller", chan);
//	}
	if (err) {
...
static int allocate_buffers(struct device *dev) {
	int i;
	/* RX BUFFERS*/
	dev_info(dev, "debug: allocating RX buffer pointer array. Size=%d\n", bufcount_rd * sizeof(struct dma_rx_buf_t));
...
//		dev_info(dev, "buffer[%02d]: logic=%p, hw=%p\n", i, dma_rx_bufs[i].buf, dma_rx_bufs[i].hwaddr);
	...
	/* TX BUFFERS*/
	dev_info(dev, "debug: allocating TX buffer pointer array. Size=%d\n", bufcount_wr * sizeof(struct dma_tx_buf_t));
...
//	dev_info(dev, "writing %d bytes\n", buf_size);
	if (buf_size > bufsize_wr) {
		...
		dev_err(dev, "trying to write %d, but max=%d. Clipped to %d.\n", buf_size, bufsize_wr, bufsize_wr);
		buf_size = bufsize_wr;
	}
...
static int create_devices2(struct xilinx_dma_device *xdev) {
	int ret = 0;
...
static int xilinx_dma_probe(struct platform_device *pdev) {
	...
	if (xdev->dma_config->dmatype == XDMA_TYPE_AXIDMA) {
		...
		xdev->common.device_prep_interleaved_dma = xilinx_vdma_dma_prep_interleaved;
	}
